...
def print_banner():
    ...
    print("\033[93mНажмите Ctrl+C для завершения\033[0m")
...
def status():
    ...
    if userbot_process is None or userbot_process.poll() is not None:
        status = "stopped"
        uptime = 0
    else:
        status = "running"
        uptime = int(time.time() - userbot_process.start_time) if hasattr(userbot_process, 'start_time') else 0
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--console":
        ...
        try:
            ...
            if os.environ.get("AUTOSTART_USERBOT", "true").lower() == "true":
